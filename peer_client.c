...
// converts the req_pdu structure into a req_buffer for transmission
void serialize() {
...
// Does the reverse thing by extracting data from the response buffer and storing it in res_pdu
void deserialize() {
...
// the menu user will be able to see
void display_menu() {
	switch(mode) {
	case 0:
		printf("===== Menu =====\n");
...
// Lists available content on the server for downloading
...
// Registers the file to a server using UDP, awaits acknowledgement, and created a child process to listen and download requests
void handle_register_content() {
	//socket init stuff
...
int listen_for_incomming_download_req(int sock_id, struct sockaddr_in sock_descriptor, char filename[11]) {
	...
	listen(sock_id, 5); // to accept incoming connections

	// an infinite loop to continously accept client requests
	while(1) {
		// when a client connect, it creates a new socket and still continues to listen for new connections
		client_len = sizeof(client);
		new_sd = accept(sock_id, (struct sockaddr *)&client, (unsigned int *) &client_len);

		// if accept returned an error otherwise, it indicates new client connection
		if (new_sd < 0) {
			...
			// if fork returns 0, the code runs in child process
			// child process handles client connections, independent of parent process
			switch(fork()){
			case 0: {
				printf("child process handling upload content\n");

				// reads data from new_sd into req_buf
				if( (n=read(new_sd, req_buf, BUFLEN) ) > 0 ) {
					// checks if the first character is 'D'
					if(req_buf[0]=='D')
						// handles the details of the file upload and the communication with the client
...
int handle_upload_content(int tcp_socket, struct sockaddr_in client, char filename[11]) {
	FILE *file_ptr;	// pointer to the opened file
	file_ptr = fopen(filename, "rb");	// tracks the number of bytes read in each chunk
	int numBytes=0, totalBytes=0, file_size;	// total number of bytes sent to the client
	int32_t tmp_file_size;
	char tmpFileBuffer[FILEDATABUFFLEN-1]; // temporary buffer for storing data before sending

	printf("\n===== Handling file download request =====\n");
	if(file_ptr == NULL){
		// if file_ptr is NULL, the file failed to open, send an error message to the client and close the socket
		write(tcp_socket, "E", 1); 
		close(tcp_socket);
	} else {
			//retreive file size
			fseek(file_ptr, 0L, SEEK_END);
			file_size = ftell(file_ptr);	// size of file in bytes
			tmp_file_size = htonl(file_size);	// converts file_size to network byte order, for data transmission
			fseek(file_ptr, 0L, SEEK_SET);	//reset file pointer
			write(tcp_socket, &file_size, sizeof(int));	// write file_size to client

			while((numBytes = fread(tmpFileBuffer, 1, sizeof(tmpFileBuffer), file_ptr)) > 0){
				file_req_buffer[0] = 'C';	// to indicate this is content data
				memcpy(file_req_buffer+1, tmpFileBuffer, sizeof(tmpFileBuffer));	//copies data from tmpFileBuffer into file_req_bffer
				write(tcp_socket, file_req_buffer, numBytes+sizeof(file_req_buffer[0]));	//then sent to client using write
				printf("%d bytes uploaded...\n", numBytes);
				totalBytes += numBytes;	//prints the total number of bytes uploaded and added to toatlBytes
			}
		fclose(file_ptr);	
	}
...
// Deregisters the file from the server, waiting for acknowledgement
